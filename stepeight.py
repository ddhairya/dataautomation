...
if Path(master.masterpath+"costcenter.xlsx").exists():
    print("Step "+step)
    ...
    for row in range(2,maxrow+1):
        ...
        for masterrow in range(2,maxmasterrow+1):
            mastercomp_data = sh1.cell(masterrow,1).value
            bakery_data = sh1.cell(masterrow,2).value
            pastry_data = sh1.cell(masterrow, 3).value
            bar_data = sh1.cell(masterrow, 4).value
            mainstore_data = sh1.cell(masterrow, 5).value
            kitchen_data = sh1.cell(masterrow, 6).value
    ...
    wb2.save(master.finalpath+"payroll.xlsx")
    status = "Done"
